...
public class Cell {
	...
	public void update(int delta) {
		...
		if (health < 0) {
			...
			if (brain instanceof BrainANN) {
				...
				// R‰knar ut score fˆr neural network fˆr att senare kunna v‰lja de b‰sta!
				...
				// Om denna cell ‰r sist och dˆr ska en check kˆras fˆr att se vilket n‰tverk
				// som fick b‰st score.
	...
	public void lateUpdate() {
		...
		if (currentCell != null) {
			// Bytte kontakt fr√•n parning till mord och gjorde att dem reproducerade med
			// sig sj√§lv ovan
			if (new Random().nextFloat() > 0.5f) {
				if (health > 60 && lifeTime > 1.5) {
					givenBirth = true;
					giveBirth(this, currentCell, NAME.split(" ")[1]); // Hitta b√§ttre l√∂sning s√• de inte parar med
																		// sig sj√§lv.
				}
